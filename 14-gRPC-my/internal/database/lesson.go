...
func (l *Lesson) FindOne(id string) (*Lesson, error) {
	var lesson Lesson
	err := l.db.QueryRow("SELECT id, name, description, content, course_id FROM lessons WHERE id = $1", id).
		Scan(&lesson.ID, &lesson.Name, &lesson.Description, &lesson.Content, &lesson.CourseID)
	if err != nil {
		return nil, err
	}
	return &lesson, nil
